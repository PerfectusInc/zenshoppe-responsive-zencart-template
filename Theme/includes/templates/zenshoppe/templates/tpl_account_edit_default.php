...
<div class="centerColumn" id="accountEditDefault">
	...
	<div class="content">
		<fieldset>
			...
			<?php echo zen_draw_input_field('firstname', $account->fields['customers_firstname'], 'id="firstname"');?>
			<br class="clearBoth" />
			...
			<?php echo zen_draw_input_field('lastname', $account->fields['customers_lastname'], 'id="lastname"'); ?>
			<br class="clearBoth" />
			<?php
              if (ACCOUNT_DOB == 'true') {
            ?>
			...
			<?php echo zen_draw_input_field('dob', zen_date_short($account->fields['customers_dob']), 'id="dob"'); ?>
            <br class="clearBoth" />
            <?php
              }
            ?>
			...
			<?php echo zen_draw_input_field('email_address', $account->fields['customers_email_address'], 'id="email-address"'); ?>
            <br class="clearBoth" />
			<label class="inputLabel" for="telephone">
				<?php echo ENTRY_TELEPHONE_NUMBER; ?>
				<?php echo zen_not_null(ENTRY_TELEPHONE_NUMBER_TEXT) ? '<span class="alert-text">' . ENTRY_FIRST_NAME_TEXT . '</span>': ''; ?>
            </label>
			<?php echo zen_draw_input_field('telephone', $account->fields['customers_telephone'], 'id="telephone"'); ?>
            <br class="clearBoth" />
			<label class="inputLabel" for="fax">
				<?php echo ENTRY_FAX_NUMBER; ?>
				<?php echo zen_not_null(ENTRY_FAX_NUMBER_TEXT) ? '<span class="alert-text">' . ENTRY_FAX_NUMBER_TEXT . '</span>': ''; ?>
            </label>
			<?php echo zen_draw_input_field('fax', $account->fields['customers_fax'], 'id="fax"'); ?>
            <br class="clearBoth" />
			<?php
              if (CUSTOMERS_REFERRAL_STATUS == 2 and $customers_referral == '') {
            ?>
			<label class="inputLabel" for="customers-referral">
				<?php echo ENTRY_CUSTOMERS_REFERRAL; ?>
            </label>
			...
			<?php
              if (CUSTOMERS_REFERRAL_STATUS == 2 and $customers_referral != '') {
            ?>
			<label for="customers-referral-readonly">
				<?php echo ENTRY_CUSTOMERS_REFERRAL; ?>
            </label>
	...
	<div class="next-prev">
        ...
        <div class="buttonRow back change_address"><?php echo '<a href="' . zen_href_link(FILENAME_ACCOUNT, '', 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_BACK , BUTTON_BACK_ALT) . '</a>'; ?></div>
    </div>
</form>
</div>