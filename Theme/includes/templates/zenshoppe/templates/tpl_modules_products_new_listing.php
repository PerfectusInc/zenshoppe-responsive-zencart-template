...
<?php
  ...
  if ($products_new_split->number_of_rows > 0) {
    ...
    $second_img_src=mb_gethoverimg_effects($products_new->fields['products_image']);
          ...
          $display_products_image = zen_image(DIR_WS_IMAGES . $products_new->fields['products_image'], $products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT) . str_repeat('', substr(PRODUCT_NEW_LIST_IMAGE, 3, 1));
        }
      } else {
...
//            $how_many++;
      ...
      if (PRODUCT_NEW_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = stripslashes(zen_trunc_string($disp_text, PRODUCT_NEW_LIST_DESCRIPTION));
        ...
        $display_products_description = '';
			...
			/*Wishlist Links*/
			if (UN_MODULE_WISHLISTS_ENABLED) { $wishlist_link_list= '<a class="wishlink" title="Add to Wishlist" href="' . zen_href_link('wishlist', 'products_id=' . $products_new->fields['products_id'] . '&action=wishlist_add_product') . '"><i class="fa fa-heart-o"></i>Add to Wishlist</a>';}else{ $wishlist_link_list='';}
		
			if (UN_MODULE_WISHLISTS_ENABLED) { $wishlist_link_grid= '<div class="wish_link"><a class="wishlink" href="' . zen_href_link('wishlist', 'products_id=' . $products_new->fields['products_id'] . '&action=wishlist_add_product') . '"><i class="fa fa-heart-o fa-lg"></i><span>Add to Wishlist</span></a></div>';}else{ $wishlist_link_grid='';}
		  	/*Wishlist Links Ends*/
			
		  	/*Add to Cart Button*/
			$addtocartbtn = zen_get_buy_now_button($products_new->fields['products_id']);
			if($addtocartbtn!=NULL){
				$addtocartbtn ='<div class="add_to_cart_link"><a class="sold_out"><i class="fa fa-ban fa-lg"></i></a>'.$addtocartbtn.'</div>';
			}
			else {
				$addtocartbtn = zen_get_buy_now_button($products_new->fields['products_id'],'<div class="add_to_cart_link"><a class="add-to-cart" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_new->fields['products_id']) . '"><i class="fa fa-shopping-cart fa-lg"></i><span>Add to Cart</span></a></div>');
			}
			
			$listview_addtocartbtn = zen_get_buy_now_button($products_new->fields['products_id'],'<a class="add-to-cart button" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_new->fields['products_id']) . '">Add to Cart</a>');
    	...
    	<div data-filter="all products" data-name="<?php echo $products_name; ?>" data-discount="0" data-price="110" class="item">
            ...
            <div class="productinfo-wrapper grid-view">
            	<div class="product_image">
        			<?php echo $ribbon;	?>
            		<a class="product-link" href="<?php echo zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_new->fields['master_categories_id']) . '&products_id=' . $products_new->fields['products_id']); ?>">
                    	<?php echo $display_products_image; ?>
                  	</a>
                    <div class="overlay">
						<div class="product-actions">
							<a class="zoom-button" data-lightbox="grid-products" data-toggle="tooltip" data-original-title="View Image" href="<?php echo DIR_WS_IMAGES . $products_new->fields['products_image'];?>"><i class="fa fa-expand fa-lg"></i></a>
							<a class="detailbutton-wrapper" data-toggle="tooltip" data-original-title="Product Detail" href="<?php echo zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . $productsInCategory[$products_new->fields['products_id']] . '&products_id=' . $products_new->fields['products_id']);?>"><i class="fa fa-link fa-lg"></i></a>
						</div>
                 		<div class="quick-actions"> <?php echo $addtocartbtn . $wishlist_link_grid ; ?>
                        	<div class="compare_link">
								<a class="comparelink" href="javascript: compareNew(<?php echo $products_new->fields["products_id"];?>, 'add')"><i class="fa fa-files-o fa-lg"></i><span>Add to Compare</span></a>
							</div>
                    	</div>
              		</div>
				</div>
                	...
                	<div class="info-left">
						<div class="col-lg-3 col-md-4 col-sm-5 col-xs-12">
							<div class="product-image">
                                ...
                                <div class="overlay">
                                    <div class="product-actions">
                                        <a class="zoom-button" data-lightbox="list-products" data-toggle="tooltip" data-original-title="View Image" href="<?php echo DIR_WS_IMAGES . $products_new->fields['products_image'];?>"><i class="fa fa-expand fa-lg"></i></a>
                                        <a class="detailbutton-wrapper" data-toggle="tooltip" data-original-title="Product Detail" href="<?php echo zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . $productsInCategory[$products_new->fields['products_id']] . '&products_id=' . $products_new->fields['products_id']);?>"><i class="fa fa-link fa-lg"></i></a>
                                    </div>
								</div>
                           	</div>
                                ...
                                <div class="listing-name-desc">
                                    <div class="listing-details">
                                        ...
                                        <div class="product-listing-actions">
                                            ...
                                            <a class="addtocompare" title="<?php echo 'Add to Compare'; ?>" href="javascript: compareNew(<?php echo $products_new->fields['products_id']; ?>, 'add')"><?php echo '<i class="fa fa-files-o"></i>Add to Compare'; ?></a>
                                        </div>
                                    </div>
                                </div>
                                <div class="product_price">
...
?>
<li><?php echo TEXT_NO_NEW_PRODUCTS; ?></li>
<?php
  }
?>
</div>
