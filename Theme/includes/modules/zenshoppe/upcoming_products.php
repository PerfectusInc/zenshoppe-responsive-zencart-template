...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  $expected_query = "select p.products_id, p.products_image, pd.products_name, pd.products_description, products_date_available as date_expected, p.master_categories_id
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $expected_query = "select p.products_id, p.products_image, pd.products_name, products_date_available as date_expected, p.master_categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd
                       where p.products_id = pd.products_id
